...
def edit(sno):
    ...
    if ('user' in session and session['user'] == params['admin_user']):
        ...
        if request.method == 'POST':
            ...
            if sno == '0':
                # print("in sno 0")
                # sn = Posts.query.max(Posts.sno).first()
                # print('Max Serial')
                # print(date.strftime("%Y-%m-%d %H:%M:%S"))
                # print(max_sno)
                max_sno = db.session.query(func.max(Posts.sno)).scalar()
                post = Posts(sno = int(0 if max_sno==None else max_sno) + 1,title= box_title,slug = slug, content = content, tagline = tline, img_file = img_file,date= date.strftime("%Y-%m-%d %H:%M:%S"))
                db.session.add(post)
                db.session.commit()
        ...
        post = Posts.query.filter_by(sno=int(sno)).first()
        date = datetime.now()
        # print("rendering_template edit.html")
        return render_template('edit.html', params = params,date = date.strftime("%Y-%m-%d %H:%M:%S"),post = post,sno = sno)
        # return redirect('/dashboard')
...
def contact():
    if request.method == 'POST':
        ...
        date = datetime.now()
        entry = Contacts(Name = name,Email = email,PhoneNum = phone,Message = message,Date = date.strftime("%Y-%m-%d %H:%M:%S"))
